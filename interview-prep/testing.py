...
class TestDataUniqueValues():
  def __init__(self): None
  @classmethod
  def get_array(self): return [1, 2, 3, 4]
  @classmethod
  def get_expected_result(self): return 0

class TestDataExactlyTwoDifferentMinimums():
  def __init__(self): None
  @classmethod
  def get_array(self): return [43, 2, 3, 2]
  @classmethod
  def get_expected_result(self): return 1

def minimum_index(seq):
  if len(seq) == 0: raise ValueError("Cannot get the minimum value index from an empty sequence")

  min_idx = 0
  for i in range(1, len(seq)):
    if seq[i] < seq[min_idx]: min_idx = i
