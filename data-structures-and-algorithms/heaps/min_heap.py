class MinHeap:
  ...
  def has_left_child(self, index): return self.get_left_child(index) < len(self.items)
  def has_right_child(self, index): return self.get_right_child(index) < len(self.items)
  def has_parent(self, index): return self.get_parent(index) >= 0
  ...
  def add(self, val):
    self.append(val) # inserted at the end
    self.heapify_up()
